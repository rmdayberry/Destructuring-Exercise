...
const remainingMiddle = ["Griffin", "Lion", "Phoenix"];

const openDoor = ({
  upper = "Alpha",
  middle = remainingMiddle[0],
  lower = "Omega",
} = {}) => {
...
//Test openDoor Function:

openDoor(doorCode); //no middle
openDoor({ upper: "Beta" }); //only the upper value
openDoor({ upper: "Beta", middle: "Eagle" }); // upper and middle values
openDoor({ upper: "Beta", middle: "Eagle", lower: "Zeta" }); // all values
openDoor(); // using all default values
...
console.log(`Translation of riddle: ${translation}`);
...
const [visibleShadow, ...hiddenShadow] = shadows;
console.log(`Visible Shadow: ${visibleShadow}`);
console.log(`Hidden Shadow: ${hiddenShadow}`);
...
const path = { direction: "West", distance: "1500 miles" };
revealPath(path);
...
//Tests:
mixPotion({ ingredient1: "Wolf Hair", ingredient2: "Wild Berries" }); // Output: Mixing Wolf Hair Wild Berries
mixPotion({ ingredient1: "Wolf Hair" }); // Output: Mixing Wolf Hair with Fireflower
mixPotion({}); // Output: Mixing Water with Fireflower
mixPotion(); // Output: Mixing Water with Fireflower
...
wiseOwlIngredients=["Dragon's Blood", "Eagle Feather","Snake Scales", "Sage", "Salt Water", "Unicorn Horn"]

const castSpell [firstIngredient, secondIngredient] = wiseOwlIngredients

console.log(`Spell- Ingredient 1: ${firstIngredient} + Ingredient 2: ${secondIngredient}`)
